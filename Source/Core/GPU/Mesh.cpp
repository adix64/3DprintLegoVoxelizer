...
bool Mesh::LoadMesh(const string& fileLocation, const string& fileName, float scale)
{
	mScaleOrig = scale;
	ClearData();
...
bool Mesh::CreateMesh(const std::vector<VertexFormat> &vertices, const std::vector<unsigned int> &indices)
{
	ClearData();
...
void Mesh::InitMesh(const aiMesh* paiMesh)
{
	...
	for (unsigned int i = 0; i < paiMesh->mNumVertices; i++) {
		...
		positions.push_back(glm::vec3(pPos->x , pPos->y, pPos->z) * mScaleOrig);

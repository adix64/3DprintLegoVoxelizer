...
namespace UtilsGPU
{
		...
		const vector<unsigned int> &indices)
	{
		...
		glBindBuffer(GL_ARRAY_BUFFER, buffers.VBO[2]);
		glBufferData(GL_ARRAY_BUFFER, sizeof(vcolors[0]) * vcolors.size(), &vcolors[0], GL_STATIC_DRAW);
		glEnableVertexAttribArray(VERTEX_ATTRIBUTE_LOC::VCOLOR);
		glVertexAttribPointer(VERTEX_ATTRIBUTE_LOC::VCOLOR, 3, GL_FLOAT, GL_FALSE, 0, 0);
